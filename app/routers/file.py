...
from app.models.entities import FilePage, File, ByCountFileResults
...
def _loadDb() -> list[dict]:
    rows = mg_run("MATCH (n:File) RETURN n AS node")
...
@router.get("/by/{field}/count", response_model=ByCountFileResults)
...
def index(request: Request, response: Response, page: Optional[int] = None, per_page: Optional[int] = None) -> Any:
    ...
    cnt = mg_run(f"MATCH (n:File) {where} RETURN count(n) AS total", params)
    total = cnt[0]["total"] if cnt else 0

    rows = mg_run(f"MATCH (n:File) {where} RETURN n AS node SKIP $skip LIMIT $limit", params)
...
def by_count(field: str, request: Request) -> Any:
    ...
    return mg_by_count("File", field, where, params)
